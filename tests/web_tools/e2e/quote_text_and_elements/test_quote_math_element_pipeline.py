# %%
"""Quote text and elements e2e: math visual element.

Why:
    Verifies that the public visual-element quote API can locate a rendered
    math element and return screenshot evidence.

Covers:
    Area: quote text and elements
    Behavior: math element lookup, bounding box evidence, annotated screenshot
    Interface: `quote_element(...)`

Checks:
    If the public element quote finds the math target, then the response
...
Examples:
    Run manually:
        uv run python -m \
            tests.web_tools.e2e.quote_text_and_elements.test_quote_math_element_pipeline

    Run as test:
        pytest \
            tests/web_tools/e2e/quote_text_and_elements/test_quote_math_element_pipeline.py
"""
...
def load_inputs(url: str = TEST_URL) -> dict[str, str]:
    """Load public inputs for the math-element quote flow."""
    return {
...
async def run_pipeline(*, element_id: str, url: str) -> VisualElementMatch | None:
    """Run the public math-element quote flow."""
...
def assert_visual_element_response(
    ...
    """Assert the stable math-element quote evidence."""
...
@pytest.mark.asyncio
async def test_quote_math_element_pipeline_hermetic(
    snapshot: SnapshotAssertion,
    e2e_site_url: str,
) -> None:
    """Verify math visual-element quoting through the public API."""
...
async def main() -> None:
    ...
    console.demo_step(
        "Scenario",
        "Finding one rendered math element on the live source page.",
    ...
    console.demo_step(
        ...
        "The public response contains a math element and screenshot metadata.",
    )
    console.print_json(actual)
    if response is not None:
        display(response.image)
    console.demo_outcome(
        "This proves `quote_element(...)` can return math-element evidence.",
    )
